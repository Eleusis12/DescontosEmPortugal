...
namespace ConsoleWebScrapperKuantoKusta
{
	public class WebScrappingPage
	{
		...
		public async Task<List<ProductDetails>> ScrapeWebsiteAsync(string siteUrl)
		{
			...
			while (true)
			{
				Colorful.Console.WriteLineStyled($"[{DateTime.Now:HH:mm:ss}] [INFO] A fazer o scrapping à pagina número: {NumberPage}", Program.M_Options.StyleConsole);
				...
				Colorful.Console.WriteLineStyled($"[{DateTime.Now:HH:mm:ss}] [INFO] Até agora foram lidos: {products.Count} produtos pertencentes à respetiva categoria", Program.M_Options.StyleConsole);
